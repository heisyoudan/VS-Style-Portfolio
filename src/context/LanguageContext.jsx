        ...
        projects: {
            ...
            items: [
                ...
                },
                {
                    ...
                    role: "作者 · 独立工程师",
                    highlight: "会话隔离。Truth 共享。Worker 可替换。项目 Truth 长存。",
                    whatIBuilt: "设计并长期维护 Maestro，一个面向可靠 AI 辅助软件工程的 Agent-agnostic 工作流控制器。它不绑定特定模型或 Agent，而是将任务目标、上下文、约束、状态与验证证据外置为持久化的项目 Truth，使不同 AI Worker 即使没有共享上一段会话历史，也能读取当前任务状态并继续工作。<br><br>Maestro 将需求转化为具有明确上下文、职责与验收标准的 Task Contract，并协调 Dev、QA、Sage 等角色完成开发、独立验证、失败回退与最终关闭。<br><br><b>工作流程：</b>Requirement → Task Contract → Dev → QA → Sage / Human Gate → Close。不同角色在隔离上下文中工作，通过共享项目状态传递 Task、Artifact、Evidence 与生命周期信息；QA 验证失败时携带证据回退 Dev，通过后继续推进。<br><br><b>核心机制：</b>Task Contract、Context Isolation、Shared Truth、Quality Gate、Atomic Rollback、Human Authority、Replaceable Workers / Provider Adapter。<br><br>Maestro 已经过长期实际项目使用和持续演进，目前作为个人 Agent Engineering 基础设施，应用于独立产品开发与商业客户交付。",
                    imgSrc: "/VS-Style-Portfolio/img/maestro.png",
